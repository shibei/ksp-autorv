...
def test_orbital_position_circular():
    ...
    r, v = orbital_position(inc=0.0, lan=0.0, arg_pe=0.0,
        sma=R_KERBIN+100_000, ecc=0.0, true_anomaly=0.0, mu=MU_KERBIN)
...
def test_cartesian_to_kepler_roundtrip():
    ...
    r_out, v_out = orbital_position(el['inc'], el['lan'], el['arg_pe'],
        el['sma'], el['ecc'], el['true_anomaly'], MU_KERBIN)
...
def test_gravity_turn_profile_shape():
    ...
    curve = gravity_turn_pitch_profile(h=10000, h_start=4000, h_end=70000, pitch_end=np.radians(5.0))
